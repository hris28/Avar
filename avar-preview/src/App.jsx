...
const data = {
  ...
  orgs: [
    ...
    { name: "Council of Regency", type: "Governing Body (Zen'tar)", leader: "Unknown", members: "Various nobles", notes: "Formed after Duke's death; allegedly involved in the Duke's death; cares only about short-term; politically warring with multiple factions" },
    ...
    { name: "The Militia", type: "Ad-Hoc Militant Force (Levies)", leader: "Zanzar (former, missing 7 years)", members: "Veterans in brotherhood lodges", notes: "Has not been active; Zanzar was so charismatic nobody could replace him; Secus wants to acquire Zanzar for this reason." },
  ...
  theories: [
    ...
    { label: "Lux = The High Elf Mage in the Time Vision", detail: "Lux is Mithe's elven mother. Lux 'joined a party cohort that soon after disappeared and their legend became more real than known.' The time-vision showed a party of 5 including a high elf woman mage — strongly implied to be Lux." },
      ...
      <div style={{
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", flexWrap: "wrap", gap: 16 }}>
            <div>
              <div style={{ fontSize: 11, letterSpacing: "0.3em", color: "#7c5c3e", textTransform: "uppercase", marginBottom: 4 }}>Campaign Compendium</div>
              <h1 style={{ margin: 0, fontSize: 28, fontWeight: "normal", color: "#e8d5b0", letterSpacing: "0.05em" }}>
                ✦ AVAR — WORLD CODEX ✦
              </h1>
            </div>
            <input
          ...
          <div style={{ display: "flex", gap: 4, marginTop: 20, flexWrap: "wrap" }}>
            {TABS.map((t, i) => (
              <button key={t} onClick={() => setTab(i)} style={{
                background: tab === i ? "#2a1a2e" : "transparent",
                border: tab === i ? "1px solid #5c3a6e" : "1px solid #2a1f1f",
                borderBottom: tab === i ? "1px solid #2a1a2e" : "1px solid #2a1f1f",
                color: tab === i ? "#d4b8e8" : "#7a6a5a",
                padding: "7px 18px",
                fontSize: 12,
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "24px 32px 48px" }}>
        ...
        {tab === 0 && (
          <div style={{ display: "grid", gap: 16 }}>
            {filtered.players.map((p, i) => (
              <div key={i} onClick={() => toggle("pc-"+i)} style={{
                background: "#0f0f1a",
                border: "1px solid #1e1e32",
                borderLeft: "3px solid #7c3a8a",
                borderRadius: 6,
                padding: "16px 20px",
                cursor: "pointer",
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
                  <div style={{ display: "flex", gap: 14, alignItems: "center" }}>
                    <span style={{ fontSize: 16, color: "#d4b8e8", fontWeight: "bold" }}>{p.name}</span>
                    <span style={{ fontSize: 11, color: "#7c5c9e", letterSpacing: "0.1em", textTransform: "uppercase" }}>{p.race} · {p.class}</span>
                  </div>
                  <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                    <span style={{ background: "#1a0a2e", border: "1px solid #3d1e5f", borderRadius: 4, padding: "2px 10px", fontSize: 11, color: "#a87cd4", letterSpacing: "0.15em" }}>ECHO: {p.keyword}</span>
                    <span style={{ fontSize: 12, color: "#6b7280" }}>Age {p.age}</span>
                  </div>
                </div>
                {expanded === "pc-"+i && (
                  <div style={{ marginTop: 14, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, borderTop: "1px solid #1e1e32", paddingTop: 14 }}>
                    <div>
                      <div style={{ fontSize: 10, color: "#5a4a3a", textTransform: "uppercase", letterSpacing: "0.2em", marginBottom: 4 }}>Appearance</div>
                      <div style={{ fontSize: 13, color: "#b8a88a", lineHeight: 1.6 }}>{p.appearance}</div>
                    </div>
                    <div>
                      <div style={{ fontSize: 10, color: "#5a4a3a", textTransform: "uppercase", letterSpacing: "0.2em", marginBottom: 4 }}>Traits & Notes</div>
                      <div style={{ fontSize: 13, color: "#b8a88a", lineHeight: 1.6 }}>{p.traits}</div>
                    </div>
                    <div style={{ gridColumn: "1/-1" }}>
                      <div style={{ fontSize: 10, color: "#5a4a3a", textTransform: "uppercase", letterSpacing: "0.2em", marginBottom: 4 }}>Connections</div>
                      <div style={{ fontSize: 13, color: "#b8a88a", lineHeight: 1.6 }}>{p.connections}</div>
                    </div>
                  </div>
                )}
              </div>
        ...
        {tab === 1 && (
          <div style={{ display: "grid", gap: 10 }}>
            {filtered.npcs.map((n, i) => (
              <div key={i} onClick={() => toggle("npc-"+i)} style={{
                background: "#0f0f16",
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
                  <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                    <span style={{ fontSize: 15, color: "#e2d9c8" }}>{n.name}</span>
                    <span style={{ fontSize: 11, color: "#6a5a4a" }}>{n.role}</span>
                  </div>
                  <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                    <span style={{ fontSize: 11, color: "#5a6a8a" }}>{n.affil}</span>
                    <span style={{ fontSize: 11, color: statusColor(n.status), border: `1px solid ${statusColor(n.status)}33`, borderRadius: 4, padding: "1px 8px" }}>{n.status}</span>
                  </div>
                </div>
                {expanded === "npc-"+i && (
                  <div style={{ marginTop: 10, borderTop: "1px solid #1a1a28", paddingTop: 10, fontSize: 13, color: "#b8a88a", lineHeight: 1.7 }}>
                    {n.notes}
                  </div>
                )}
              </div>
        ...
        {tab === 2 && (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: 14 }}>
            {filtered.locations.map((l, i) => (
              <div key={i} style={{
                background: "#0d1210",
                border: "1px solid #1a2820",
                borderTop: "2px solid #2e6e4a",
                borderRadius: 6,
                padding: "16px 18px",
              }}>
                <div style={{ fontSize: 14, color: "#9ee8b8", marginBottom: 4 }}>{l.name}</div>
                <div style={{ fontSize: 10, color: "#3a6a50", textTransform: "uppercase", letterSpacing: "0.15em", marginBottom: 8 }}>{l.type}</div>
                <div style={{ fontSize: 12, color: "#8aaa98", lineHeight: 1.65 }}>{l.desc}</div>
              </div>
            ))}
          </div>
        )}
        ...
        {tab === 3 && (
          <div style={{ display: "grid", gap: 12 }}>
            {filtered.orgs.map((o, i) => (
              <div key={i} onClick={() => toggle("org-"+i)} style={{
                background: "#100d0f",
                border: "1px solid #1e1820",
                borderLeft: "3px solid #5e3a5e",
                borderRadius: 6,
                padding: "14px 18px",
                cursor: "pointer",
              }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
                  <span style={{ fontSize: 15, color: "#e0c8e8" }}>{o.name}</span>
                  <span style={{ fontSize: 10, background: tagColors[o.type] || "#1e1e1e", border: "1px solid #2e2e3e", borderRadius: 4, padding: "2px 10px", color: "#b898d8", letterSpacing: "0.1em", textTransform: "uppercase" }}>{o.type}</span>
                </div>
                {expanded === "org-"+i && (
                  <div style={{ marginTop: 12, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, borderTop: "1px solid #1e1820", paddingTop: 12 }}>
                    <div>
                      <div style={{ fontSize: 10, color: "#5a4a5a", textTransform: "uppercase", letterSpacing: "0.2em", marginBottom: 4 }}>Leader</div>
                      <div style={{ fontSize: 13, color: "#b8a8c8", lineHeight: 1.6 }}>{o.leader}</div>
                    </div>
                    <div>
                      <div style={{ fontSize: 10, color: "#5a4a5a", textTransform: "uppercase", letterSpacing: "0.2em", marginBottom: 4 }}>Known Members</div>
                      <div style={{ fontSize: 13, color: "#b8a8c8", lineHeight: 1.6 }}>{o.members}</div>
                    </div>
                    <div style={{ gridColumn: "1/-1" }}>
                      <div style={{ fontSize: 10, color: "#5a4a5a", textTransform: "uppercase", letterSpacing: "0.2em", marginBottom: 4 }}>Notes</div>
                      <div style={{ fontSize: 13, color: "#9a9ab8", lineHeight: 1.7 }}>{o.notes}</div>
                    </div>
                  </div>
                )}
              </div>
        ...
        {tab === 4 && (
          <div style={{ display: "grid", gap: 14 }}>
            <div style={{ fontSize: 11, color: "#5a4a3a", letterSpacing: "0.2em", textTransform: "uppercase", marginBottom: 4 }}>
              ⚠ These are player theories — not confirmed canon
            </div>
            {filtered.theories.map((t, i) => (
              <div key={i} style={{
                background: "#100e08",
                border: "1px solid #2a2010",
                borderLeft: "3px solid #c87c20",
                borderRadius: 6,
                padding: "16px 20px",
              }}>
                <div style={{ fontSize: 14, color: "#e8c87c", marginBottom: 8, fontStyle: "italic" }}>✦ {t.label}</div>
                <div style={{ fontSize: 13, color: "#b8a070", lineHeight: 1.75 }}>{t.detail}</div>
              </div>
            ))}
          </div>
        )}
